( function( $ ){
    ...
    var $backgrounds = $( "<ul class='sm-slider'></ul>" ),
        ...
        init : function( settings ) {
            ...
            $backgrounds.html(this.mkhtml( options.src )).prependTo(".sm-sliderOuter");
        ...
        mkcss: function( bgCount , duration , fade  ) 
        {
            ...
            keyframes = "@keyframes imageAnimation {0% {opacity: 0;animation-timing-function: ease-in;}{0}% {opacity: 1;}{1}% {opacity: 1;}{2}% {opacity: 0; animation-timing-function: ease-out;transform: scale(1.17) translateY(-4%) rotateZ(0);}100% { opacity: 0;transform: scale(1.17) translateY(0) rotateZ(0); }} @keyframes titleAnimation {0% { opacity: 0;transform: translateY(-200px); }{0}% { opacity: 1;transform: translateY(0%); }{1}% { opacity: 1;transform: translateY(0%); }{3}% { opacity: 0; transform: scale(0.4) translateY(100%); }100% { opacity: 0; }}".format(fadeInPercentile,remainPercentile,fadeOutPercentile,fadeOutPercentile_title),
            otherCSS = ".sm-slider:after { content: '';background: none;z-index: 0; }.sm-slider li span { width: 100%;height: 100%;position: absolute;bottom: 0px;left: 0px;color: transparent;background-size: cover;background-position: 50% 50%;background-repeat: none;opacity: 0;z-index: 0;animation: imageAnimation {1}s linear infinite 0s; }".format(this.options.overlay,total_time),
